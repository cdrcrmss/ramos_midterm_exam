...
<div class="max-w-4xl mx-auto p-6">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('users.profile', $followedUser) }}" class="font-bold text-gray-900 hover:underline">
                                        {{ $followedUser->display_name }}
                                    </a>
                                    @if($followedUser->username)
                                        <span class="text-gray-500">{{ '@' . $followedUser->username }}</span>
                                    @endif
                                </div>
                                @if($followedUser->bio)
                                ...
                                <div class="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                                    <span>{{ $followedUser->tweets->count() }} tweets</span>
                        ...
                        @auth
                            @if($followedUser->id !== auth()->id())
                                <div class="flex items-center space-x-3">
                                    <form action="{{ route('follow.toggle', $followedUser) }}" method="POST" class="inline">
                                        @csrf
                                    ...
                                    {{-- <a href="{{ route('messages.conversation', $followedUser) }}" 
    ...
    @else
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            @if($user->id === auth()->id())
                <h3 class="text-xl font-semibold text-gray-900 mb-2">You're not following anyone yet</h3>
                <p class="text-gray-600 mb-6">When you follow people, they'll show up here</p>
            ...
            @else
                <h3 class="text-xl font-semibold text-gray-900 mb-2">{{ $user->display_name }} isn't following anyone yet</h3>
                <p class="text-gray-600">When they follow people, they'll show up here</p>
            @endif
        </div>
    @endif
</div>
